...
<title>Pak Indus Ice Factory- View Cheques Given</title>
    ...
    <body>
        ...
        <div id="wrapper">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">View Cheques Given</h4>
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active">View Cheques Given</li>
                                    </ol>
            ...
            @endif   
                                            ...
                                            <h4 class="mt-0 header-title">View Cheques Given</h4>
                                            <p class="text-muted m-b-30">These are all the cheques given.</p>
                                              ...
                                              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                ...
                                                <tbody>
                                                     @foreach($cheque_given as $cg)
                                                <tr>
                                                    <td><a href="{{ route('single.cheque.details', ['id'=>$cg->id]) }}">{{$cg->id}}</a></td>
                                                    <td>
                                                    
                                                    {{$cg->dealer_name}}
                                                    
                                                    </td>
                                                    <td>{{date('d-m-Y', strtotime($cg->date))}}</td>
                                                    
                                                    <td>{{date('d-m-Y', strtotime($cg->clearing_date))}}</td>
                                                    
                                                    <td><a href="{{ asset("attachments/cheque/{$cg->cheque_attachment}") }}"> {{$cg->cheque_number}}</a></td>
                                                    <td>{{$cg->cheque_amount}}</td>
                                                      <td> 
                                                          @if($cg->cheque_status=='Cleared')
                                                        <h5 style="color: green">{{$cg->cheque_status}}</h5>
                                                          @elseif($cg->cheque_status=='Pending' || $cg->cheque_status=='Cancelled')
                                                           <h5 style="color: red">{{$cg->cheque_status}}</h5>
                                                              @else
                                                              <h5 style="color: orange">{{$cg->cheque_status}}</h5>
                                                              @endif
                                                    </td>
                                                      ...
                                                      <td style="text-align: center;">
                                                         ...
                                                         <form action="{{route('update.cheque.status')}}" method="POST">
                                                            {{ csrf_field() }}
                                                            <input type="hidden" value="{{$cg->id}}" name="cheque_id">
